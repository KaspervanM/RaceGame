...
from classes.NN import NNev, NNHuman, NNLongest, generate_random_NNev_model, gen_mutant
...
def empty_room():
				...
				if f"{i},{x}" in models:
					if all(f"{i},{x}" != elem[0] for elem in best):
						...
						if randint(0, int(nCars*(best[index % top][1]+1)**1.5)) == 0:
							print('Model mutated')
							mutated_model = gen_mutant(generate_random_NNev_model(best_model, mutation_rate), mutation_rate)
	...
	if generation % 10 == 0:
		if exists(modelpath): rmtree(modelpath, ignore_errors=True)
...
if any(inp == ans for ans in ['no', 'n', 'No', 'N']):
	print('ok, no then')
	model_scores = {}
	for modnum in range(nModels):
		model = generate_random_NNev_model()
		print(modnum)
		for x in range(nCarsPerModel):
			models[f"{modnum},{x}"] = gen_mutant(gen_mutant(model, 1.), 1.)
else:
	with open(modelpath+'gen.log', 'r') as logfile:
		data = literal_eval(logfile.read())
		generation = data[0]
		start_time = int(time.time())-data[1]
	for modnum in range(50):
		if exists(f"{modelpath}type{modnum}"):
			for x in range(100):
				if exists(f"{modelpath}type{modnum}{fs}model{x}.h5"):
					models[f"{modnum},{x}"] = load_model(f"{modelpath}type{modnum}{fs}model{x}.h5")
	model_scores = {}
set_car()
update()
...
def on_key_press(key, modifiers):
	global b, inf, showmodel, show_cars
	...
	for count, elem in enumerate(car):
		if elem:
			if show_cars:
				...
				if showmodel and car[0]: car[0].show_model()
				pyglet.text.Label('{:d}, {:d}'.format(elem.IFLocal.modelindex[0], elem.IFLocal.modelindex[1]),
				 font_name='Times New Roman', font_size=10, x=elem.x, y=elem.y).draw()
			if (elem.check_collision(elem.hitbox) or (not (Human and count == 0) and elem.unusedframes > 100)):
				collision(count)
